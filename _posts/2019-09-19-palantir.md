...
によって計算することができる．$R_{ij}$は中間状態$i$から最終状態$j$（$nW-b+j$番目の状態）を表すので，$B_{ij}=\sum_kF_{ki}R_{kj}$は中間状態$i$から（いつか）最終状態$j$に到達する確率を表わしている．

$\boldsymbol{B}_{i}$ は $\sum_j B_{ij}=1$ となる多項分布である．また，最終状態から最終状態への分岐確率は，明らかに
